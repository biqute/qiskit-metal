...
class OpenToGroundRounded(QComponent):
    """A sample_shapes open to ground termination. Functions as a pin for auto drawing.
    ...
    def make(self):
        ...
        circle = draw.Point(0,0).buffer(p.width / 2)
        half_box = draw.box(0, -p.width / 2, p.width / 2, p.width / 2)
        rounded_termination = circle.intersection(half_box)

        circle_gap = draw.Point(0,0).buffer(p.width / 2 + p.gap)
        half_box_gap = draw.box(0, -p.width / 2  - p.gap, p.width / 2 + p.gap, p.width / 2 + p.gap)
        rounded_termination_gap = circle_gap.intersection(half_box_gap)
        
        # Rotates and translates the connector polygons (and temporary port_line)
        polys = [rounded_termination, rounded_termination_gap, port_line]
        polys = draw.rotate(polys, p.orientation, origin=(0, 0))
        polys = draw.translate(polys, p.pos_x, p.pos_y)


        [rounded_termination, rounded_termination_gap, port_line] = polys

        # Subtracts out ground plane on the layer its on
        self.add_qgeometry('poly', {'rounded_termination': rounded_termination},
                           layer=p.layer)
        self.add_qgeometry('poly', {'rounded_termination_gap': rounded_termination_gap},
                           subtract=True,
                           layer=p.layer)
